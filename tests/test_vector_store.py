...
class FakeEmbeddingService:
    KEYWORDS = ("python", "machine learning", "react", "aws", "nlp")
...
def test_build_job_search_text_includes_core_fields() -> None:
    ...
    assert "Required skills: Python, AWS, Machine Learning" in search_text
...
def test_candidate_vector_store_builds_and_searches(tmp_path: Path) -> None:
    ...
    assert "Python" in results[0].skills or "AWS" in results[0].skills
